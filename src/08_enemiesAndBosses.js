...
class DroneEnemy extends Enemy {
  ...
  drawObj(ctx) {
     ...
     // Drone core eye
     ctx.fillStyle = (this.state === 'CHASE') ? '#FF0000' : '#4444FF';
     ctx.beginPath();
     ctx.arc(this.x + 10, this.y + 10, 6 + Math.sin(Date.now()/100)*2, 0, Math.PI*2);
     ctx.fill();
  }
}
...
class HunterEnemy extends Enemy {
   ...
   drawObj(ctx) {
     ctx.save();
     ctx.translate(this.x + 10, this.y + 19);
...
class TurretEnemy extends Enemy {
   ...
   drawObj(ctx) {
     ...
     ctx.fillRect(this.x, this.y + 4, 32, 16);

     // Energy core
     ctx.fillStyle = '#11AAFF';
     ctx.fillRect(this.x + 10, this.y + 4, 12, 12);
     
     // Alien overgrowth vines
     ctx.strokeStyle = '#00FF44'; ctx.lineWidth = 2;
     ctx.beginPath();
     ctx.moveTo(this.x + 16, this.y + 20); 
     ctx.quadraticCurveTo(this.x + 4, this.y + 10, this.x + 8, this.y);
     ctx.stroke();
